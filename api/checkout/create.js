...
export default async function handler(req, res){
  ...
  try{
    ...
    const body = { request: { amount: Number(amountMinor), currency: 'RUB', description, return_url: finalReturnUrl, notification_url: notificationUrl, test: false } };

    const resp = await fetch(overpayUrl, { method:'POST', headers:{ 'Authorization':auth, 'Content-Type':'application/json' }, body: JSON.stringify(body) });
